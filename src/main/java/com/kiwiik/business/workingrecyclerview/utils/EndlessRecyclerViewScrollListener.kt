...
abstract class EndlessRecyclerViewScrollListener : RecyclerView.OnScrollListener {
    ...
    constructor(layoutManager: LinearLayoutManager, itemsPerPage : Int) {
        this.mLayoutManager = layoutManager
        this.itemsPerPage = itemsPerPage
    }

    constructor(layoutManager: GridLayoutManager, itemsPerPage : Int){
        this.mLayoutManager = layoutManager
        visibleThreshold *= layoutManager.spanCount
        this.itemsPerPage = itemsPerPage
    }

    constructor(layoutManager: StaggeredGridLayoutManager, itemsPerPage : Int) {
        this.mLayoutManager = layoutManager
        visibleThreshold *= layoutManager.spanCount
        this.itemsPerPage = itemsPerPage
    }
    ...
    override fun onScrolled(view: RecyclerView, dx: Int, dy: Int) {
        ...
        if (!loading && lastVisibleItemPosition + visibleThreshold > totalItemCount) {
            var itemsToValidate = itemsPerPage
            if(currentPage > 0){
                itemsToValidate = itemsPerPage * (currentPage + 1)
            }
            if(totalItemCount == itemsToValidate) {
                currentPage++
